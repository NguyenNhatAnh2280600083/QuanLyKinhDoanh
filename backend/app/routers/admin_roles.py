...
@router.get("/roles", response_model=List[RoleOut], dependencies=[Depends(require_role("admin"))])
async def list_roles(db: Session = Depends(get_db)):
    return RoleService.list_roles(db)


@router.post("/roles", response_model=RoleOut, dependencies=[Depends(require_role("admin"))])
async def create_role(role_in: RoleCreate, db: Session = Depends(get_db)):
    return RoleService.create_role(db, role_in)


@router.put("/roles/{role_id}", response_model=RoleOut, dependencies=[Depends(require_role("admin"))])
...
@router.get("/permissions", response_model=List[PermissionOut], dependencies=[Depends(require_role("admin"))])
async def list_permissions(db: Session = Depends(get_db)):
    return RoleService.list_permissions(db)


@router.put("/roles/{role_id}/permissions", response_model=RoleOut, dependencies=[Depends(require_role("admin"))])
